...
{ 
    ...
    {     
        ...
        if($FeatureStatus.DefinitionId -eq $null)
        {
            ...
            Write-Host -f DarkMagenta "Feature not Activated!" 
        }
        else
        {
            Write-host -f DarkGreen "Feature already Actived !" 
        }
    } 
    Catch
    {
...
foreach($key in $features.keys)
{
    ...
    Features_Status -SiteCollURL $SiteCollURL -UserName $cred.UserName -Password $cred.Password -FeatureGuid $FeatureGuid
