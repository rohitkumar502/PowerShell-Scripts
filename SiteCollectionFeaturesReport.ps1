...
#Config Parameters
$ReportOutput="G:\PowerShell\FeaturesReport.csv"
...
$SiteCollURL = "https://fastekart.sharepoint.com/sites/CCEPTeamSite"
...
foreach($key in $features.keys)
{
    ...
    $FeatureGuid= [System.Guid] ($features[$key]) 

    #Send the Result to CSV 
    $Result = new-object PSObject
    $Result| add-member -membertype NoteProperty -name "Feature Name " -Value $key

    $stats = Features_Status -SiteCollURL $SiteCollURL -UserName $cred.UserName -Password $cred.Password -FeatureGuid $FeatureGuid

    $Result | add-member -membertype NoteProperty -name "Status" -Value $stats
...
Write-Host "Site Collections Features Report Generated Successfully!" -f Green